...
class NativeHost extends EventEmitter<{
  ...
  pending: Map<
    number,
    { resolve: (response: NativeResponse) => void; reject: (error: unknown) => void }
  >;
  stopping: boolean;
  failure: Error | null;
  ...
  terminalDecoder: Decoder<NativeMessage>;
  ...
  resolveReady!: (message: NativeReady) => void;
  rejectReady!: (error: unknown) => void;
  readyMessage?: NativeReady;
  ...
  constructor(binary: string, env: NodeJS.ProcessEnv) {
    ...
    this.terminalDecoder = new Decoder<NativeMessage>((message) => {
      if (
        message.type !== 'event' ||
        !(
        ...
        throw new Error('Invalid native terminal record');
      if (
        !Number.isSafeInteger(message.terminalSeq) ||
        message.terminalSeq !== this.terminalSequence + 1
      )
        throw new Error('Out-of-order native terminal frame');
    ...
    this.terminalServer = net.createServer((socket) => {
      ...
      socket.on('end', () => {
        ...
        if (!this.stopping && !this.exitInfo && this.child?.exitCode === null)
          this.fail(new Error('Native terminal channel ended'));
        this.maybeExit();
      });
    ...
    this.ready = new Promise<NativeReady>((resolve, reject) => {
  ...
  armStartupDeadline() {
    ...
    this.timer = setTimeout(
      () => this.fail(new Error('Native core startup made no progress for 30 seconds')),
      30000,
    );
  }
  attachChild() {
    this.decoder = new Decoder<NativeMessage>((message) => this.receive(message));
    ...
    this.child.stderr.on('data', (data) => {
      ...
      if (Buffer.byteLength(this.stderrText) > 64 * 1024 * 1024)
        this.fail(new Error('Oversized native diagnostic record'));
    });
    ...
    this.child.on('exit', (code, signal) => {
      ...
      const error = new Error(`Native core exited (${code ?? signal})`);
  ...
  receive(message: NativeMessage) {
    if (message.type === 'initializing') {
      ...
      if (
        ...
        return this.fail(new Error('Invalid native initialization progress'));
    ...
    if (message.type === 'ready') {
      if (message.protocol !== 1) return this.fail(new Error('Incompatible native protocol'));
      if (message.terminalChannel !== true)
        return this.fail(new Error('Native core lacks a dedicated terminal channel'));
  ...
  async invoke(command: string, args: object = {}, type = 'request'): Promise<NativeResponse> {
    await this.ready;
    if (this.failure || this.stopping || this.child.exitCode !== null || this.child.signalCode)
      throw new Error('Native core is unavailable');
    if (this.pending.size >= 4096) throw new Error('Too many pending native requests');
  ...
  async stop() {
    ...
    this.stopping = true;
    clearTimeout(this.timer);
    this.rejectReady(new Error('Native core stopped'));
