...
const binary = require('node:path').join(__dirname, 'fixtures/native-child.js');

function host(mode: string) {
  return new NativeHost(binary, { ...process.env, PROBE_MODE: mode });
}
...
test('protocol mismatch and truncated stdout terminate the child', { timeout: 8000 }, async () => {
  for (const mode of ['bad-version', 'truncated']) {
    ...
    try {
      await bounded(exited);
      assert.match(h.failure!.message, mode === 'bad-version' ? /Incompatible/ : /Truncated/);
...
test(
  ...
  async () => {
   ...
   const {NativeHost}=require(${JSON.stringify(require.resolve('../native.js'))});
   const h=new NativeHost(${JSON.stringify(mode === 'missing' ? '/missing-phase3-native' : binary)},{...process.env,PROBE_MODE:'echo'});
   h.on('failure',()=>{});
...
test(
  ...
  async (t) => {
    ...
    t.mock.method(global, 'setTimeout', (...[fn, ms, ...args]: Parameters<typeof setTimeout>) =>
      realSetTimeout(fn, ms === 30000 ? 120 : ms, ...args),
    );
    const progressing = host('initializing');
    let ready = false;
    progressing.ready.then(() => (ready = true));
    try {
      await new Promise((r) => realSetTimeout(r, 180));
